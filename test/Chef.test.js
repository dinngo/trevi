...
describe('MiniChefV2', function() {
  ...
  describe('PendingSushi', function() {
    it('PendingSushi should equal ExpectedSushi', async function() {
      ...
      let timestamp2 = (await ethers.provider.getBlock(log2.blockNumber))
        .timestamp;
      let timestamp = (await ethers.provider.getBlock(log.blockNumber))
        .timestamp;
      let expectedSushi = BigNumber.from('10000000000000000').mul(
  ...
  describe('Harvest', function() {
    it('Should give back the correct amount of SUSHI and reward', async function() {
      ...
      expect(
        (await this.chef.userInfo(0, this.alice.address)).rewardDebt
    ...
    it('Harvest for SUSHI-only pool', async function() {
      ...
      await this.chef.harvest(0, this.alice.address);
      expect(await this.sushi.balanceOf(this.alice.address)).to.be.equal(
        expectedSushi
      );
    });
  });
